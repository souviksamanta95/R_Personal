...
bayes.model <- naiveBayes(DEATH_EVENT ~ ., data=train.df)
bayes.model

pred.train1 <- predict(bayes.model,train.df,type = "class")
confusionMatrix(pred.train1, as.factor(train.df$DEATH_EVENT),positive="1")
# We got an accuracy of 72.4%
# we got sensitivity of 30.67%

# generate confusion matrix for validation data
pred.valid1 <- predict(bayes.model,valid.df,type = "class")
confusionMatrix(pred.valid1, as.factor(valid.df$DEATH_EVENT),positive="1")
# We got an accuracy of 73.3%
# we got sensitivity of 23.81%
