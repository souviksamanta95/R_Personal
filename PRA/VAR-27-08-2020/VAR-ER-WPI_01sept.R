...
VARselect(d_df[,1:3],lag.max=6,type="const") [["selection"]]

#Vector autoregression with lags set according to results of lag optimisation. 
var_est <- VAR(d_df[,1:3], p=3, type = "const", season=NULL)
summary(var_est)
...
grangertest(d_WPI ~ d_ER, order = 1)
grangertest(d_ER ~ d_WPI, order = 1)

grangertest(d_WPI ~ d_MS, order = 1)
grangertest(d_MS ~ d_WPI, order = 2)
