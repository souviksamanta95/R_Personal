...
##Estimation
fit1 <- Arima(data$Close, order=c(1,1,0), seasonal = list(order = c(0, 0, 0), period=5))
summary(fit1)
b1<-tsdiag(fit1)

fit2 <- Arima(data$Close, order=c(0,0,0), seasonal = list(order = c(1, 1, 0), period=5))
summary(fit2)
b2<-tsdiag(fit2)

fit3 <- Arima(data$Close, order=c(0,0,0), seasonal = list(order = c(3, 1, 0), period=5))
summary(fit3)
b3<-tsdiag(fit3)
...
fit4 <- Arima(data$Close, order=c(0,0,0), seasonal = list(order = c(4, 1, 0), period=5))
summary(fit4)
b3<-tsdiag(fit4)
