...
##Estimation
fit1 <- Arima(df$Close, order=c(0,0,0), seasonal = list(order = c(1, 1, 1), period=5))
fit1
summary(fit1)
fit2 <- Arima(df$Close, order=c(0,1,0), seasonal = list(order = c(1, 1, 0), period=5))
fit2
summary(fit2)
...
##Diagnostics
tsdisplay(res)
Box.test(residuals(fit3), lag=16, fitdf=4, type="Ljung")
b2<-tsdiag(fit3)
b2
